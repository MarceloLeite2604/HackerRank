...
func serviceLane(n int32, cases [][]int32) []int32 {
	...
	result := make([]int32, len(cases)/2)

	for caseIndex := 0; caseIndex < len(cases); caseIndex += 2 {
		widths := cases[caseIndex]
		entry := cases[caseIndex+1][0]
		exit := cases[caseIndex+1][1]

		minWidth := int32(-1)
		for widthsIndex := entry; widthsIndex <= exit; widthsIndex++ {
			sectionWidth := widths[widthsIndex]
			if minWidth == -1 || sectionWidth < minWidth {
				minWidth = sectionWidth
			}
		}
		result[caseIndex/2] = minWidth
	}
...
func main() {
	...
	for i := 0; i < int(t); i++ {
		...
		cases = append(cases, width, casesRow)
	}
